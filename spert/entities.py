...
class Document_plus:
    ...
    def __init__(self, doc_id: int, id: string, text: string, tokens: List[Token], entities: List[Entity], relations: List[Relation],
                 encoding: List[int]):
        self._doc_id = doc_id  # ID within the corresponding dataset
        self._orig_id = id
        self._text = text
...
class Dataset(TorchDataset):
    ...
    def create_document_plus(self, id, text, tokens, entity_mentions, relations, doc_encoding) -> Document:
        document = Document_plus(self._doc_id, id, text, tokens, entity_mentions, relations, doc_encoding)
